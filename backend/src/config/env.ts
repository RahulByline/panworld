...
function buildDatabaseUrl(e: z.infer<typeof EnvSchema>) {
  if (e.DATABASE_URL && e.DATABASE_URL.length > 0) return e.DATABASE_URL;
...
function parseExpiryToSeconds(exp: string) {
...
export type Env = Omit<z.infer<typeof EnvSchema>, "DATABASE_URL" | "WEB_ORIGIN"> & {
  DATABASE_URL: string;
  CORS_ORIGINS: string[];
...
const accessSeconds =
  parsed.JWT_ACCESS_EXPIRY ? parseExpiryToSeconds(parsed.JWT_ACCESS_EXPIRY) ?? parsed.JWT_ACCESS_TTL_SECONDS : parsed.JWT_ACCESS_TTL_SECONDS;
const refreshSeconds =
  parsed.JWT_REFRESH_EXPIRY ? parseExpiryToSeconds(parsed.JWT_REFRESH_EXPIRY) ?? parsed.JWT_REFRESH_TTL_SECONDS : parsed.JWT_REFRESH_TTL_SECONDS;
