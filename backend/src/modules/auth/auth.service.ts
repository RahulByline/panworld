...
async function compareToken(token: string, tokenHash: string) {
  return bcrypt.compare(token, tokenHash);
}

export const authService = {
  login: async (email: string, password: string, meta: { userAgent?: string; ip?: string }) => {
  ...
  refresh: async (refreshToken: string) => {
    let claims: { uid: string; typ: string };
    try {
    ...
    const match = await Promise.any(
      tokens.map(async (t) => ((await compareToken(refreshToken, t.token_hash)) ? t : null)),
    ).catch(() => null);
  ...
  logout: async (refreshToken: string | null) => {
    ...
    try {
      ...
      if (claims.typ !== "refresh") return { ok: true };
